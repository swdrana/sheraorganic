...
import Pagination from "../common/others/Pagination";
import StarRating from "../common/others/StartRating";
import Loading from "../common/others/Loading";
import Link from "next/link";
import ProductCard from "../common/card/ProductCard";
import ProductListCard from "../common/card/ProductListCard";

const ShopGridProduct = ({
  pageCount,
  handlePageChange,
  filteredProducts,
  itemsPerPage,
        ...
        <div className="shop-grid">
          <div className="listing-top d-flex align-items-center justify-content-between flex-wrap gap-3 bg-white rounded-2 px-4 py-5 mb-6">
            <p className="mb-0 fw-bold">
              Showing {itemsPerPage * currentPage}-
              {itemsPerPage * (currentPage + 1)} of{" "}
            ...
            <div className="listing-top-right text-end d-inline-flex align-items-center gap-3 flex-wrap">
              {/* <div className="number-count-filter d-flex align-items-center gap-3">
                                        ...
                                        <button onClick={increaseItemsPerPage}>Increase</button>
                                    </div> */}
              <div className="number-count-filter d-flex align-items-center gap-3">
                <label className="fw-bold fs-xs text-dark flex-shrink-0">
                  Show:
                </label>

                <button type="" className="" onClick={decrementItems}>
                  -
                </button>
                ...
                <button type="button" className="" onClick={incrementItems}>
                  +
                </button>
              </div>
              <div className="select-filter d-inline-flex align-items-center gap-3">
                ...
                <select
                  className="form-select fs-xxs fw-medium theme-select select-sm"
                  onChange={(e) => setSortValue(e.target.value)}
                  value={sortValue}
                >
                  <option value="" hidden>
                    {" "}
                    {/* Set a blank value for the placeholder */}
                    Price
                  </option>
                  <option value="a_zorder">A _ Z Order</option>{" "}
                  {/* Add values for each option */}
                  <option value="z_aorder">Z _ A Order</option>
                  <option value="low_highprice">Low _ High Price</option>
                  <option value="high_lowprice">High _ Low Price</option>
                </select>
              </div>
              <a
                href="#"
                onClick={() => setGridBtn("gridView")}
                className={`grid-btn ${gridBtn === "gridView" ? "active" : ""}`}
              >
                <svg
                  width="17"
                  height="16"
              ...
              <a
                href="#"
                onClick={() => setGridBtn("listView")}
                className={`grid-btn ${gridBtn === "listView" ? "active" : ""}`}
              >
                <svg
                  width="21"
                  height="16"
